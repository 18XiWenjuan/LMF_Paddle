...
save_model_value=$(func_parser_value "${lines[6]}")
    ...
    for use_gpu in ${use_gpu_list[*]}; do
        ...
        if [ ${use_gpu} = "False" ] || [ ${use_gpu} = "cpu" ]; then
            for batch_size in ${batch_size_list[*]}; do
                ...
                command="${_python} ${_script} ${use_gpu_key}=${use_gpu} ${set_batchsize} ${set_benchmark} > ${_save_log_path} 2>&1 "
                echo $command
                eval $command
...
if [ ${MODE} = "whole_infer" ]; then
    ...
    for gpu in ${gpu_list[*]}; do
        ...
        for trainer in ${trainer_list[*]}; do
            ...
            set_save_model=$(func_set_params "${save_model_key}" "${save_model_value}")
            if [ ${#gpu} -le 2 ];then  # train with single gpu
                cmd="${python} ${run_train} ${set_save_model} ${set_epoch} ${set_pretrain} ${set_batchsize} ${data_path_key} ${data_path_value}"
            ...
            if [ ${run_export} != "null" ]; then
                ...
                func_inference "${python}" "${inference_py}" "./test_tipc/logs"
